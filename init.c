...
double Init_Rmin_Function(double r);  // The function to minimize to get Ea and ka
...
double Init_Rmin_Function(double r) {
    /* 
    Function to minimize = V(r) + 1/(2 m r^2)
    */
    double f, mass;

    mass = PARAM_DATA.mass;
    f = RadialEqFunctions_V(r) + 1.0/(2.0*mass*r*r);  // f = V(r) + 1/(2 m r^2)
